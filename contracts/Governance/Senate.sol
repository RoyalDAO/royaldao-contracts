...
abstract contract Senate is Context, ERC165, EIP712, ISenate {
  ...
  constructor(
    ...
    require(_Chancellor != address(0), "Senate::Invalid Chancellor address(0)");
  ...
  ) external override ifSenateClosed {
    for (uint256 idx = 0; idx < _tokens.length; idx++) {
      if (
        IERC165(_tokens[idx]).supportsInterface(type(ISenatorVotes).interfaceId)
      ) {
        if (!tokens.contains(_tokens[idx])) {
          memberCounter.increment();
          memberId[_tokens[idx]] = SafeCast.toUint32(memberCounter.current());
          idMember[SafeCast.toUint32(memberCounter.current())] = _tokens[idx];

          tokens.add(_tokens[idx]);
        }
      } else if (
        IERC165(_tokens[idx]).supportsInterface(type(IVotes).interfaceId)
      ) {
  ...
  function _acceptToSenate(address _token) internal {
    ...
    if (IERC165(_token).supportsInterface(type(ISenatorVotes).interfaceId)) {
      if (!tokens.contains(_token)) {
        ...
        try ISenatorVotes(_token).setSenate(this) {} catch {}
        //add New Member to Senate List of Members
        tokens.add(_token);
      }
    } else if (IERC165(_token).supportsInterface(type(IVotes).interfaceId)) {
  ...
  function writeMemberToSenateBooks(address member) private {
    //get owners list
    ISenatorVotes.senateSnapshot[] memory _totalSuply = ISenatorVotes(member)
      .getSenateSnapshot();
